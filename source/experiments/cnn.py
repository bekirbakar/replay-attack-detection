...
x_train, y_train, _, _, batch_size, _ = load_features(feature_list[feature], 'train')
x_test, y_test, test_labels, indexes, _, test_file_list = load_features(feature_list[feature], 'dev')
...
while (epoch < 10000) and (not done_looping):
    metrics = keras_model.fit(x_train, y_train, epochs=1, batch_size=batch_size, shuffle=True, verbose=0)
    predictions = keras_model.predict(x_test, verbose=0, batch_size=batch_size)
    score_file = create(predictions, labels=test_labels, file_list=test_file_list, indexes=None)
    this_eer = scoring.calculate_eer(score_file)
    dev_eer.append(this_eer)
    print('Epoch={}, val_eer={}, accuracy(train) = {:,.4}'.format(epoch, this_eer, metrics.history['acc'][0]))
