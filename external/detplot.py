...
def pavx(y):
    ...
    for j in range(1, n):
        ...
        while (ci >= 1) & (ghat[numpy.max(ci - 1, 0)] >= ghat[ci]):
            ...
            ghat[ci - 1] = ghat[ci - 1] + (length[ci] / nw) * (
                    ghat[ci] - ghat[ci - 1])
            length[ci - 1] = nw
            ci -= 1
...
def plotseg(xx, yy, box, dps):
    ...
    if (xx[0] < box.left) | (xx[1] > box.right) | (yy[1] < box.bottom) | (
            yy[0] > box.top):
        x = numpy.array([])
        y = numpy.array([])
    else:
...
def rocchdet(tar, non,
    ...
    """
    assert ((pfa_min > 0) & (pfa_max < 1) & (pmiss_min > 0) & (
            pmiss_max < 1)), 'limits must be strictly inside (0,1)'
    assert ((pfa_min < pfa_max) & (
            pmiss_min < pmiss_max)), 'pfa and pmiss min and max values are not consistent'
...
class DetPlot:
    ...
    def create_figure(self, idx=0):
        ...
        ax.set_yticklabels(self.__plotwindow__.__yticklabels__, size='large')
        if not self.__title__ == '':
            mpl.title(self.__title__)
        mpl.grid(True)
        mpl.xlabel('Yanlış Red Oranı [%]', size='x-large')
        mpl.ylabel('Yanlış Kabul Oranı [%]', size='x-large')
    ...
    def set_system_from_scores(self, scores, key, sys_name=''):
        ...
        """
        assert isinstance(scores,
                          Scores), 'First argument should be a Score object'
    ...
    def plot_rocch_det(self, idx=0, style='color', target_prior=0.001,
                       plot_args=''):
        ...
        tmp = numpy.array([sigmoid(__logit__(target_prior)),
                           sigmoid(-__logit__(target_prior))])
    ...
    def plot_mindcf_point(self, target_prior, idx=0, plot_args='ok',
        ...
        mindcf, pmiss, pfa, prbep, eer = fast_minDCF(self.__tar__[idx],
                                                     self.__non__[idx],
                                                     __logit__(target_prior),
                                                     True)
        if (pfa < self.__plotwindow__.__pfa_limits__[0]) | (
                pfa > self.__plotwindow__.__pfa_limits__[1]):
            logging.warning(
                'pfa of %f is not between %f and %f mindcf point will not be plotted.',
                format(pfa),
            ...
            logging.warning(
                'pmiss of %f is not between %f and %f. The mindcf point will not be plotted.',
    ...
    def plot_DR30_fa(self,
        ...
        :param legend_string: Optional. A string to describe this curve
            in the legend.
        """
        assert isinstance(plot_args, tuple) & (
                len(plot_args) == 3), 'Invalid plot_args'
        ...
        if (pfaval < pfa_min) | (pfaval > pfa_max):
            logging.warning(
                'Pfa DR30 of %f is not between %f and %f Pfa DR30 line will not be plotted.',
    ...
    def plot_DR30_miss(self,
        ...
        if (pmissval < pmiss_min) | (pmissval > pmiss_max):
            logging.warning(
                'Pmiss DR30 of is not between %f and %f Pfa DR30 line will not be plotted.',
